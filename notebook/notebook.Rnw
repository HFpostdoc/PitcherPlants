...
\item Initial analyses of the Hawley and Molly datasets
\end{itemize}
...
###mark
i <- 9
test <- lapply(h99.tpl,function(x,i,times) gews(x[,i],times), i=i, times=h99.tpl.date[[1]])
times <- h99.tpl.date[[k]]
series <- h99.tpl[[k]][,9]
out <- gews(series,times)
