### R code from vignette source 'vignettes/KEGGgraph/inst/doc/KEGGgraphApp.Rnw'
...
toyDrawnRefine <- makeNodeRenderAttrs(toyDrawn,
                                      fill=ifelse(nodeType=="gene", "lightblue", "orange"),
                                      shape=ifelse(nodeType=="gene",
                                        "ellipse","rectangle"))
...
sprintf("%2.2f%% genes differentially-expressed",
          %mean(isDiffExp)*100)
...
logcol <- cols[incol]; logcol[logfcs==0] <- "darkgrey";
logcol[undetected] <- "yellow"
names(logcol) <- names(logfcs)
nA <- makeNodeAttrs(g, fillcolor=logcol, label='', width=10,
                    height=1.2)
...
upNeighbor <- nodes(upSub)[sapply(neighborhood(upSub, nodes(upSub)),
                                  length)>0]
...
upnA <- makeNodeAttrs(upSub, fillcolor=logcol[nodes(upSub)],
                      label=upSymbol, fixedsize=TRUE, width=10, height=10, font=20)
